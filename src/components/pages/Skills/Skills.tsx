...
const Skills = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (categoryMatch) {
      return mainIcons.filter((icon: any) => 
  ...
  const circularChartData = useMemo(() => {
    if (selectedPieCategory) {
      return filteredData
        .filter((icon: any) => icon.category.toLowerCase() === selectedPieCategory.toLowerCase())
      ...
      if (!isMobile && searchTerm.trim()) {
        ...
        if (categoryMatch) {
          const categorySkills = filteredData.filter((icon: any) => 
            icon.category.toLowerCase() === categoryMatch.value.toLowerCase()
          );
          
          if (categorySkills.length > 0) {
            const averagePercentage = Math.round(
              categorySkills.reduce((sum: number, skill: any) => sum + skill.percentage, 0) / categorySkills.length
            );
            ...
            return [{
              ...
              skills: categorySkills.sort((a: any, b: any) => b.percentage - a.percentage)
            }];
          }
        } else {
          return filteredData
        ...
        return categoryOptions
          ...
          .map(category => {
            ...
            const averagePercentage = categorySkills.length > 0 
              ? Math.round(categorySkills.reduce((sum: number, skill: any) => sum + skill.percentage, 0) / categorySkills.length)
              : 0;
